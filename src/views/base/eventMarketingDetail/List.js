...
export default function EventMarketingDetail() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${API_URL}/api/eventMarketingDetail/getAll`)
  ...
  const handleAddEventMarketingDetail = () => {
    navigate('add')
  }
      ...
      {loading ? (
        <Paper style={{ width: '100%', boxShadow: 'rgba(99, 99, 99, 0.2) 0px 2px 8px 0px' }}>
          <Box display="flex" justifyContent="space-between" alignItems="center">
            <Button
              onClick={handleAddEventMarketingDetail}
              sx={{ m: 2 }}
              variant="contained"
          ...
          <Box display="flex" justifyContent="space-between" alignItems="center">
            ...
            >
              DANH SÁCH CHI TIẾT SỰ KIỆN MARKETING
            </Typography>
            {/* <TextField
              variant="outlined"
          ...
          <TablePagination
            ...
            component="div"
            count={eventMarketingDetail.length}
