...
const ImageSaleRightHome = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${API_URL}/api/imageRightHome/getDetail?id=${id}`)
  ...
  const handleImageSaleRightHome = async () => {
    try {
      const response = await axios.post(`${API_URL}/api/imageSaleRightHome/update`, formData, {
    ...
    >
      ...
      <Grid container justifyContent="center" alignItems="center">
        <Grid item xs={12} sx={{ px: 4, my: { lg: 0, xs: 4 } }}>
          <Button onClick={handleImageSaleRightHome} variant="contained" color="primary">
            Cập nhật
          </Button>
        </Grid>
      </Grid>
...
export default ImageSaleRightHome
